...
{
  int	   t;
  _/* x86 FPU float-to-int replaced */
  return t;
}
#else
...
ILINE void cryMemcpy(void* Dst, const void* Src, int n) {
	...
	{
#if !defined(LINUX)
		for (int p = 0; p < PREFNTA_BLOCK; p+=64) { __builtin_prefetch((char *) src + p); }
#endif
...
#if !defined(LINUX)
	for (int p = 0; p < n; p+=64) { __builtin_prefetch((char *) src + p); }
#endif
	memcpy(dst, src, n);
}
