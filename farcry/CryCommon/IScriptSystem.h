...
#if defined(WIN64) || defined(LINUX) || defined(VITA)
	inline void EndCall(char *&sRet) {
		const char* tmp = sRet; EndCall(tmp); sRet = const_cast<char*>(tmp);
	}
#endif
...
//#endif
	...
	*/
	...
	inline bool GetCurrentKey(char* &sVal) {
		const char* tmp = sVal; bool r = GetCurrentKey(tmp); sVal = const_cast<char*>(tmp); return r;
	}
	inline bool GetCurrent(char* &sVal) {
		const char* tmp = sVal; bool r = GetCurrent(tmp); sVal = const_cast<char*>(tmp); return r;
	}
	inline bool GetAt(int nIdx,char* &sVal) {
		const char* tmp = sVal; bool r = GetAt(nIdx, tmp); sVal = const_cast<char*>(tmp); return r;
	}
#endif
...
struct IFunctionHandler
{
	...
	*/
	...
	inline bool GetParam(int nIdx, char * &s) {
		const char* tmp = s;
		bool r = GetParam(nIdx, tmp);
		s = const_cast<char*>(tmp);
		return r;
	}
#endif
...
#if !defined(VITA)  // ULONG_PTR == unsigned int == HSCRIPTFUNCTION on 32-bit; avoid duplicate overload
...
#if !defined(VITA)  // ULONG_PTR == unsigned int on 32-bit; would duplicate EndFunction(HSCRIPTFUNCTION)
