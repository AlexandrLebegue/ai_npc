...
//-------------------------------------------
//-- portability functions for PS Vita (ARM)
//-------------------------------------------
#if defined(VITA)
#define ILINE inline
ILINE void cry_sincosf(float a, float* p) { p[0]=(float)cosf(a); p[1]=(float)sinf(a); }
ILINE void cry_sincos(double a, double* p){ p[0]=cos(a); p[1]=sin(a); }
ILINE float cry_sinf(float x)             { return sinf(x); }
ILINE float cry_cosf(float x)             { return cosf(x); }
