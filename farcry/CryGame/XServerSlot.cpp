...
void CXServerSlot::OnXServerSlotConnect(const BYTE *pbAuthorizationID, unsigned int uiAuthorizationSize)
{
	...
	memcpy(m_vAuthID, pbAuthorizationID, std::min((unsigned int)64, uiAuthorizationSize));
	m_bAuthIDSize = std::min((unsigned int)64, uiAuthorizationSize);
}
	...
	unsigned int uiGlobalIDSize )
{
	...
	if (pGlobalID && uiGlobalIDSize)
	{
		...
		memcpy(m_vGlobalID, pGlobalID, std::min((unsigned int)64, uiGlobalIDSize));
