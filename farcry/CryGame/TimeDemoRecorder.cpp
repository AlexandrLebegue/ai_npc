...
#if defined(VITA) || defined(LINUX)
	{
		const char* slash = strrchr(m_file.c_str(), '/');
		const char* dot   = strrchr(m_file.c_str(), '.');
		size_t fnlen = dot ? (size_t)(dot - (slash ? slash+1 : m_file.c_str())) : strlen(slash ? slash+1 : m_file.c_str());
		const char* base = slash ? slash+1 : m_file.c_str();
		size_t dirlen = slash ? (size_t)(slash+1 - m_file.c_str()) : 0;
		strncpy(dir, m_file.c_str(), dirlen); dir[dirlen] = '\0';
		strncpy(fname, base, fnlen); fname[fnlen] = '\0';
		snprintf(path_buffer, _MAX_PATH, "%s%s.log", dir, fname);
	}
#else
