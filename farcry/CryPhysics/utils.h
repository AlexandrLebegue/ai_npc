...
#elif defined(LINUX32) || defined(VITA)
const int	imag	= (23+127)<<23 | 1<<22;
...
inline int64 GetTicks() {
	SceRtcTick t; sceRtcGetCurrentTick(&t); return (int64)t.tick;
}
#endif
...
#elif defined(VITA)
	// VITA: is_valid without the generic template (avoids operator| on float)
	inline bool is_valid(float op) { return op*op>=0 && op*op<1E30f; }
	inline bool is_valid(int op) { return true; }
	inline bool is_valid(unsigned int op) { return true; }
	template<class dtype> inline bool is_valid(const dtype &op) { return true; }
#endif
...
#define DoBreak { DEBUG_BREAK; }
	...
	#include "validator.h"
#elif defined(VITA)
	// Validators disabled on Vita (no runtime physics validation needed for port)
	#define VALIDATOR_LOG(pLog,str)
	#define VALIDATORS_START bool validate( const char *strSource, ILog *pLog, const vectorf &pt,\
		IPhysicsStreamer *pStreamer, void *param0, int param1, int param2 ) { bool res=true;
	#define VALIDATOR(member)
	#define VALIDATOR_NORM(member)
