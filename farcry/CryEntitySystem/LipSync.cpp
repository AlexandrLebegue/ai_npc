...
bool CLipSync::LoadRandomExpressions(const char *pszExprScript, bool bRaiseError)
{
	...
	if (m_pScriptSystem->ExecuteFile(pszExprScript, true, false))
	{
		...
		{
			const char *base = strrchr(pszExprScript, '/');
			base = base ? base+1 : pszExprScript;
			strncpy(filename, base, sizeof(filename)-1); filename[sizeof(filename)-1]=0;
			char *dot = strrchr(filename, '.'); if (dot) *dot = 0;
		}
#else
