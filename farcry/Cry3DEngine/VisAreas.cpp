...
bool CVisArea::UpdatePortalCameraScissor(CCamera & cam, list2<Vec3d> * lstVerts, bool bMergeFrustums)
{
  ...
  vMin2d.x = std::max(vMin2d.x,0.0f);
  vMin2d.y = std::max(vMin2d.y,0.0f);
  vMax2d.x = std::min(vMax2d.x,(float)GetRenderer()->GetWidth());
  vMax2d.y = std::min(vMax2d.y,(float)GetRenderer()->GetHeight());
    ...
    if(nDLMask & (1<<nId))
    {
      ...
      if(pDLight->m_pOwner->GetEntityVisArea() == this)
      {
        ...
        { // not first time - merge
          int nMaxX = std::max((int)(pDLight->m_sX + pDLight->m_sWidth),  (int)CurCamera.m_ScissorInfo.x2);
          int nMaxY = std::max((int)(pDLight->m_sY + pDLight->m_sHeight), (int)CurCamera.m_ScissorInfo.y2);
          pDLight->m_sX = (short)std::min((int)pDLight->m_sX, (int)CurCamera.m_ScissorInfo.x1);
          pDLight->m_sY = (short)std::min((int)pDLight->m_sY, (int)CurCamera.m_ScissorInfo.y1);
