...
//	m_nLastVisibleFrameId = GetFrameID();
  ...
  //changed by ivo
    ...
    float fTranspPlus1 = std::max(0.0f, r1/nChunksNum-0.5f)*2;
    float fTranspPlus2 = std::max(0.0f, r2/nChunksNum-0.5f)*2;
      ...
      float fAlpha;
			...
			tmp.color.bcolor[3] = (unsigned char)fastftol_positive(std::min(255.0f,fAlpha*fA));
