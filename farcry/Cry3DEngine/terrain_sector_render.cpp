...
void CSectorInfo::AddLowResSectorIndex(int _x, int _y, int _step, int _lod)
{
  unsigned short id = _x/_step*(CTerrain::GetTerrainSize()/_step+1) + _y/_step;
	...
	m_nLowResTerrainIdxRange[_lod][0] = (uint)std::min((uint)m_nLowResTerrainIdxRange[_lod][0],(uint)id);
	m_nLowResTerrainIdxRange[_lod][1] = (uint)std::max((uint)m_nLowResTerrainIdxRange[_lod][1],(uint)id);
